...
export default function BrandsPage() {
  ...
  function handleLogoChange(e) {
    ...
    if (!acceptedTypes.includes(file.type)) {
      showNotification('error', 'Please upload an image in PNG, JPEG, JPG, or GIF format');
      return;
    }
  ...
  return (
    <div>
      ...
      {/* <div className="mb-6 flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <button 
            ...
            className={`p-2 rounded ${viewMode === 'grid' ? 'bg-blue-500 text-white' : 'bg-gray-black'}`}
          >
          ...
          <button 
            ...
            className={`p-2 rounded ${viewMode === 'table' ? 'bg-blue-500 text-white' : 'bg-gray-black'}`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4 text-black">{editingId ? 'Edit Brand' : 'Add New Brand'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-black mb-1">Brand Name</label>
                <input
                  id="name"
                  type="text"
                  value={formData.name}
              ...
              <div>
                <label htmlFor="websiteUrl" className="block text-sm font-medium text-black mb-1">Website URL</label>
                <input
                  id="websiteUrl"
                  type="url"
              ...
              <div>
                <label htmlFor="logo" className="block text-sm font-medium text-black mb-1">Brand Logo (PNG, JPEG, JPG, GIF)</label>
                <input
                  id="logo"
                  type="file"
                  ref={fileInputRef}
                  ...
                  className="w-full p-2 border rounded focus:ring-2 focus:ring-blue-300 focus:border-blue-500 text-black"
                />
                <p className="mt-1 text-sm text-black">Please upload an image file in PNG, JPEG, JPG, or GIF format</p>
              </div>
            </div>
      ...
      {isLoading ? (
        ...
        brands.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="mt-2 text-gray-600">No brands found. Add your first brand using the form above.</p>
          </div>
        ) : viewMode === 'grid' ? (
